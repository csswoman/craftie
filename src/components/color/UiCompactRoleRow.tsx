  ...
  return (
    <li>
      <button
        ...
        className="group flex min-h-10 w-full items-center gap-2 rounded-md px-1.5 text-left transition-colors duration-200 hover:bg-line-soft focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-forest/25 dark:hover:bg-line-soft"
      >
        <span
          ...
          aria-hidden="true"
        />
        <span className={`truncate text-sm font-semibold ${unassigned ? 'text-attention' : 'text-ink'}`}>
          {label}
        </span>
        {!unassigned ? (
          <span className="ml-auto shrink-0 rounded-full bg-line-soft px-1.5 py-0.5 text-[0.625rem] font-semibold text-muted opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-visible:opacity-100 dark:bg-surface-raised">
            {origin}
          </span>
        ...
        {unassigned ? (
          ...
          <span className="w-[4.9rem] shrink-0 text-right font-mono text-[0.71875rem] tabular-nums text-faint">
